export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!order?.name || !order?.phone || !order?.address || !Array.isArray(order?.items) || !order.items.length) {
      return new Response(JSON.stringify({ ok:false, error:'Please complete all order details.' }), { status:400, headers:{'content-type':'application/json'} });
    }
    ...
    return new Response(JSON.stringify({ ok:false, error:'Could not place the order.' }), { status:400, headers:{'content-type':'application/json'} });
  }
}